...
function Order_Form_Endplazierung({ person_data, spiel_tipps }) {
    ...
    const handleSubmitEndplazierung = async (event) => {
        ...
        try {
            const response = await fetch('https://svs-tippspiel.de/api/saveEndplazierung.php', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    const mail_data = {
                        ...
                        "tipps": spiel_tipps,
                        "endplazierung": {
                            "Endplazierung_1": inputs.Endplazierung_1,
                            "Endplazierung_2": inputs.Endplazierung_2
                        }
                    }

                    console.log(mail_data);

                    const response = await fetch('https://svs-tippspiel.de/api/mail.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
            ...
            <form onSubmit={handleSubmitEndplazierung}>
                <FormControl fullWidth sx={{ maxWidth: "500px", margin: '20px 0px 20px 0px' }}>
                    ...
                    <Card style={{ margin: '10px' }} >
                        ...
                        <strong>Bezirksliga Ost Qualirunde</strong>
                    </Card>
                    ...
                    <Card style={{ margin: '10px' }} >
                        ...
                        <CardContent alignItems='center'>
                            <Grid container alignItems="center" >
                                <Grid item sx={{ width: 250 }}>
                                    <Typography> Endplatzierung 2. Mannschaft</Typography>
                                </Grid>
                                <TextField sx={{ width: 50 }} keyboardType="numeric" onChange={(event) => handleChange(event)} name='Endplazierung_2' value={inputs.Endplazierung_2} pattern="[0-9]*" required />.
                            </Grid>
                        </CardContent>
                        <strong>Kreisliga A II Riß</strong>
                    </Card>
...
const Tippabgabe_Endplazierung = () => {
...
export default Tippabgabe_Endplazierung;